...
bool OCRepPayloadAddResourceTypeAsOwner(OCRepPayload* payload, char* resourceType)
{
    ...
    if (payload->types)
    {
        ...
        cur->next->value = OICStrdup(resourceType);
        if (!cur->next->value)
        {
            OICFree(cur->next);
            cur->next = NULL;
    ...
    else
    {
        ...
        payload->types->value = OICStrdup(resourceType);
        if (!payload->types->value)
        {
            OICFree(payload->types);
            payload->types = NULL;
            return false;
        }
        return true;
...
bool OCRepPayloadAddInterfaceAsOwner(OCRepPayload* payload, char* iface)
{
    ...
    if (payload->interfaces)
    {
        ...
        cur->next->value = OICStrdup(iface);
        if (!cur->next->value)
        {
            OICFree(cur->next);
            cur->next = NULL;
    ...
    else
    {
        ...
        payload->interfaces->value = OICStrdup(iface);
        if (!payload->interfaces->value)
        {
            OICFree(payload->interfaces);
            payload->interfaces = NULL;
            return false;
        }
        return true;
    }
